...
#this is new, I have multiple routers doing same test so these default routes need to be removed. 

bash -c "$(echo "sudo route del -net 0.0.0.0 gw 10.50.0.254")"
bash -c "$(echo "sudo route del -net 0.0.0.0 gw 10.50.0.253")"
bash -c "$(echo "sudo route del -net 0.0.0.0 gw 10.50.0.252")"
bash -c "$(echo "sudo route del -net 0.0.0.0 gw 10.50.0.251")"
...
##Save SQLITE3, just unhash if still want it on pihole dashboard, i dont care for it as im logging everything to either influx or mysql. 
...
#Insert to MySql
echo "insert into speedtest values (NULL, '${start}', '${stop}', '${from}', '${from_ip}', '${server}', ${server_dist}, ${server_ping}, ${download}, ${upload}, '${share_url}');" | mysql -uroot -ppassword speedtest
